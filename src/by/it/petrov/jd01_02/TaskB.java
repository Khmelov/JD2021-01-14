    ...
    private static void step1() {
        ...
        for (int i = 0; i < twoDimArray.length; i++) { // Цикл генерации матрицы
            for (int j = 0; j < twoDimArray[0].length; j++) {
                ...
                if (i == 0 && j == 0) {   // Определение первого значения
                    twoDimArray[i][j] = 1;
                }

                if (i == 0 && j != 0) {   // Определение значения для элементов первой строчки (исключая первый элемент)
                    twoDimArray[i][j] = twoDimArray[i][j - 1] + 1;
                }

                if (i != 0 && j != 0) {   // Определение элементов строк, не являющихся первыми
                    twoDimArray[i][j] = twoDimArray[i][j-1] + 1;
                }

                if (i != 0 && j == 0) {   // Определение элементов строк не первой строки для элементов первых столбцов

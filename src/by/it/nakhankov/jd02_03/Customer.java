...
class Customer extends Thread implements CustomerAction {
    ...
    Customer(int number, Market market) {
        super("Customer №" + number);

        this.market = market;
        market.getDispatcher().enteredCurrentCustomer();
    }
    ...
    public void run() {
        ...
        goToQueue();
        goOut();
        market.getDispatcher().completeCurrentCustomer();
    }
    ...
    public void goToQueue() {
        ...
        synchronized (this) {
            market.getQueueCustomers().add(this);
            waitFlag = true;
            while (waitFlag)
                try {
                    this.wait();
